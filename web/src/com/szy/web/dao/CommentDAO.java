...
public class CommentDAO
{
	...
	public ArrayList<Comment> getComments(int nid,int startCid,int count) throws SQLException
	{
		...
		sql = "select comid,nid,ptime,region,content,supportcount,opposecount from t_comment where nid=? and deleted=false order by ptime desc limit ?,?";
	...
	{
		long count = 0;
		sql = "select count(comid) as count from t_comment where nid=?";
